...
5. 물리 상수 실측 3종은 **여러 세션째 계속 미완료**다 — `T_stop`(정지 지연·오버슈트, `grippers-host-mac/host/t_stop_probe.py`가 이미 있음, 아직 실행한 적 없음), 모터 워치독 발동 시간, `identify_target` 6클래스 왕복 지연. `pi_capture/mac/analyze_stop.py`·`analyze_watchdog.py`로 분석.
...
- `T_stop`(정지 지연·오버슈트) 실측 — 도구(`t_stop_probe.py`)까지 새로 만들어
  뒀는데 아직 한 번도 실행 안 됨.
