...
class Camera:
    ...
    _acc_n: int = 0
    _bad_frames: int = 0
    _lock_warned: bool = False                 # 고정 실패 경고를 한 번만 찍기 위한 플래그
    ...
    def load(cls, name: str, index: int) -> "Camera":
        ...
        if path.exists():
            d = np.load(path)
            # K 는 픽셀 단위다 — 촬영 해상도와 실행 해상도가 다르면 초점거리가
            # 그 비율만큼 통째로 어긋난다. 예전에는 확인 없이 그냥 읽어서,
            # 720p 로 찍은 fx≈930 을 1080p 에 쓰면 참값 1395 대비 33% 틀린 채로
            # 경고 하나 없이 돌았다. 스케일 보정으로 때우지 않고 거부한다 —
            # 조용히 맞춰 주면 잘못된 캘리브레이션을 계속 쓰게 된다.
            size = d["image_size"] if "image_size" in d.files else None
            if size is not None:
                cw, ch = (int(v) for v in size)
                if (cw, ch) != (cfg.IMG_W, cfg.IMG_H):
                    print(f"⚠️ {name}: 캘리브레이션은 {cw}x{ch} 에서 찍혔는데 "
                          f"지금은 {cfg.IMG_W}x{cfg.IMG_H} 로 실행합니다.")
                    print(f"   초점거리가 {cfg.IMG_W / cw:.2f} 배 어긋나므로 이 파일을 "
                          f"쓰지 않고 근사값으로 돌립니다.")
                    print(f"   → python calibrate_camera.py --cam {index} 를 "
                          f"이 해상도에서 다시 돌리세요.")
        ...
        if cfg.EXTRINSIC_LOCK_FRAMES and self.n_floor >= cfg.MIN_FLOOR_MARKERS:
            ...
            if self._acc_n >= cfg.EXTRINSIC_LOCK_FRAMES:
                ...
                if self._solve_from(avg, per_id):
                    ...
                    self._acc.clear()
                    return True
                # 고정 실패 — 누적을 반드시 비우고 처음부터 다시 모은다.
                #
                # 안 비우면 _acc_n 이 31, 32, ... 로 끝없이 올라가고(화면에
                # "locking 31/30" 이 찍힌다) _acc 리스트도 계속 자라서,
                # 매 프레임 점점 커지는 배열의 평균을 다시 계산하게 된다.
                if not self._lock_warned:
                    self._lock_warned = True
                    print(f"\n⚠️ {self.name}: 외부파라미터 고정 실패 — "
                          f"재투영 {self.reproj_px:.2f} px > "
                          f"{cfg.MAX_EXTRINSIC_REPROJ_PX} px "
                          f"(바닥 {self.n_floor}/4). 다시 모읍니다.")
                self._acc.clear()
                self._acc_n = 0
...
def box_entrance(name: str) -> tuple[float, float]:
    """상자 "입구" 중앙의 (x, y). BOXES 에는 중심이 적혀 있어서 반 길이를 뺀다."""
    x, y, _ = box_pose(name)
    return float(x), float(y - cfg.BOX_L / 2.0)


def box_approach_target(name: str) -> tuple[float, float]:
    """상자 앞에 설 때 겨냥할 "마커 중심" 목표 좌표.

    ⚠️ box_pose() 를 그대로 주행 목표로 쓰면 안 된다. 그건 상자 '중심'이라
       로봇이 상자 안으로 들어가려 한다. 두 값을 빼야 실제 정지 지점이 된다:

           상자 중심 y
             − BOX_L/2                    → 입구
             − BOX_APPROACH_FROM_ENTRANCE → 마커 중심이 설 자리  ← 주행 목표

    뒤 항은 현장에서 로봇을 원하는 자리에 세워 놓고 카메라로 직접 잰 값이다.
    '차체 앞 간격' 과 '마커~범퍼' 로 쪼개지 않는다 — 제어기가 필요한 건 합뿐이고,
    쪼개려면 범퍼 위치를 자로 재야 하는데 현장에서 어렵다.

    두 상자 모두 뒤쪽 벽에 붙어 작업 영역(−y) 쪽으로 열려 있으므로 로봇은
    y 가 작은 쪽에서 접근한다. 배치가 바뀌면 이 함수와 그 값을 다시 잡아야 한다.
    """
    x, ey = box_entrance(name)
    return x, float(ey - cfg.BOX_APPROACH_FROM_ENTRANCE)
