...
가상 카메라 2대를 실측 배치(높이 1.62 m · 후퇴 0 m · 좌우 0.9 m ·
...
를 한 번에 확인할 수 있다.
    ...
    python selftest.py
    python selftest.py --setback 0.0 --height 1.62 --tilt 42.8
"""
...
#
# 대신 이 배치(setback=0, height=1.62)로 시뮬레이션을 돌려보면 tilt 를
# 40°~78° 사이 어디에 둬도 작업 영역 사각지대가 0 이다 (측정: 30~85°
# 스윕, 39° 이하/80° 이상부터 손실 시작). C270 때(42.8°에서 5°만 더
# 숙여도 8% 손실)보다 훨씬 관대해졌다 — HFOV 가 넓어지고 후퇴가 0이라
# 생긴 여유다. 아래 42.8 은 예전 값을 그대로 가져온 것으로, 이 안전범위
# 안에 들어가고 selftest 전체가 깨끗이 통과하는 값이라 기본값으로 남겨
# 뒀다 — 그렇다고 실제로 잰 각도는 아니다. 각도기 앱으로 재서 정확한
# 값을 넣으면 더 정확히 검증된다.
#
...
# FAIL 로 뜬다. 정상이다.
TILT_DEG = 42.8     # 하향각 — 미실측, 예전 값을 그대로 사용 (안전범위 40~78° 안)
CAM_H = 1.620       # 바닥에서 렌즈까지
