...
class ArmDriverNode(Node):
    ...
    # 지시로 처음엔 45도로 잡았다가, 45도 안에서 부호 반전·재진입 버그를
    # 고친 뒤 두 번의 실기(지향오차 +6.2도·+25.3도)가 연속 성공하자
    # "60도로 더 넓혀도 될것 같아"(사용자 지시, 같은 날)로 60도로 올렸다.
    # 그런데 같은 날 밤 실제 미션 실행에서 60도 근처까지 크게 튼 보정이
    # 드랍 지점을 바구니 왼쪽 벽 쪽으로 몰아 "가끔 너무 왼쪽이라
    # 아슬아슬"한 경우가 나와 45도로 되돌렸는데, 45도 이내(+22.6도·
    # -21.9도 등)로 재검증한 그날 밤 마지막 실행에서도 같은 지적이 남아
    # 20도까지 낮췄다가, 사용자 지시로 다시 30도로 올렸다("한계각 30도로
    # 고쳐줘" — 20도가 지나치게 좁았다고 판단). Host 쪽
    # basket_target.NO_ROTATION_* (이미 충분히 좋은 자세면 아예 안 돌린다)
    # 로 애초에 회전 빈도 자체를 줄여 뒀으니, 이 한계각은 "그래도 돌아야
    # 하는 나머지 경우"를 위한 여유로 본다.
    # 위 MAX_BASE_YAW_OFFSET_RAD(15도)와 일부러 공유하지 않는다: 그 값은
    # GRASP 하강 시 턱 폭 허용치(±76mm)에서 역산된, 물리적으로 근거 있는
    # 한계라 안 건드린다. 여기(드랍 직전 300mm 자세)는 물체를 이미 확실히
    # 쥔 채 바닥과 접촉이 없는 전혀 다른 상황이라 같은 한계를 쓸 이유가
    # 없다.
    #
    # 2026-09-07 — basket_target.facing_offset_from_square() 버그 수정
    # (위치가 목표영역 경계와 겹치면 지향오차가 0으로 퇴화하던 것) 이후
    # 실제 잔여 오차가 처음으로 정확히 보고되면서, 같은 실행에서 두 번
    # (+41.4도·+36.4도) 30도 한계에 걸려 거부됐다 — 보정 없이 그 각도
    # 그대로 투하됨. 60도 근처에서 "너무 왼쪽" 문제가 재현된 전례가 있어
    # 그 구간까지 되돌리진 않되, 사용자 지시로 45도로 다시 올린다("45도로
    # 해줘") — 60도 시기 실측(6.2도·25.3도 성공)과 45도 시기 실측(22.6도·
    # -21.9도, 다만 이때도 "너무 왼쪽" 지적이 있었다)을 감안해 45도 이내
    # 보정은 이 프로젝트에서 실기 검증된 적이 있는 구간이다.
    #
    # ⚠️ 45도 자체가 이번에 새로 검증된 상한은 아니다 — 45도 근처 보정은
    # 과거에도 "가끔 너무 왼쪽" 지적을 받은 적이 있다는 걸 감안하고 쓸 것.
    # 45도를 넘는 보정은 거부되어 servo1이 안 움직이고 드랍한다.
