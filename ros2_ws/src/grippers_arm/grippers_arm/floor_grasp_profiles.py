...
TAUGHT_POSITION_LIMITS = {
    ...
    # 2026-09-07: 팀원(sysy009)이 VLA 브랜치에서 이 Min_Angle_Limit을
    # 1140->1090으로 실측 변경했다("그리퍼가 안 닫히던 건 소프트웨어
    # 제한이었다", vla-into-baseline 커밋). restore_taught_offsets.py로
    # 물리 EEPROM을 다시 읽어 1090임을 확인 — 이 팔의 실제 상태이자
    # 팀원이 고친 값이므로, 되돌리는 대신 이 교시 기준값 자체를 갱신한다.
    6: (1090, 2090),    # gripper — 2026-09-07 갱신(원래 1140, 2026-09-01 사고 레지스터)
}
