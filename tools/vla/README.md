...
**하드웨어 없이 검증했습니다** (`tests/test_vla_*.py`, 82개)
...
- Pi 5 CPU 에서 SmolVLA 추론이 몇 ms 인지. 청크 50스텝이 50Hz 에서 1초
  분량이므로, 추론이 1초를 넘으면 팔이 계속 붙들기 상태에 들어갑니다.
  그때는 `send_hz` 를 낮춰 청크를 길게 쓰는 편이 낫습니다.
