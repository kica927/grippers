...
| 문서 | |
|---|---|
| [`COLLECTION_PLAN.md`](COLLECTION_PLAN.md) | **몇 개를 · 무엇을 바꿔 가며 찍는가** · 남은 9일 배치 |
| `preflight.sh` | 파이에 접속하자마자 돌리는 점검 (읽기 전용) |
...
**하드웨어 없이 검증했습니다** (`tests/test_vla_*.py`, 102개)
...
- 파이용 축소 점검(`selftest.py`)이 본체와 같은 계약을 본다
- `preflight.sh` 가 상태를 바꾸지 않는다
